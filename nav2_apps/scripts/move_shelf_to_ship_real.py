...
class MoveShelfToShipNode(Node):
  ...
  def find_reflective_indices(self, scan):
    ...
    for i in range(n):
      ...
      # Skip low intensity values
      if intensity < 3400.0:
        continue

      # Compute the angle of this laser beam 
      theta = scan.angle_min + i * scan.angle_increment

      # Convert range + angle into x/y position
      x = r * math.cos(theta)
      y = r * math.sin(theta)

      inside_shelf_search_zone = (
        0.2 <= x <= 2.0 and -0.8 <= y <= 0.8
      )

      # Rejecet shiny point outside the prpbalbe shelf area
      if not inside_shelf_search_zone:
        continue
  ...
  # Tranform laser [X,Y] to robot_odom  
  ...
  def creep_forward(self, distance_m, speed_mps=0.10):
    ...
    # Safty timeout 
    max_time_sec = distance_m / max(1e-6, speed_mps) + 3.0
  ...
  def lift_shelf(self):
    ...
    for i in range(5):
      ...
      self.get_logger().info(f"Publish empty elevator UP tigger ({i + 1} / 5)")
      time.sleep(0.1)
  ...
  def lower_shelf(self):
    ...
    for i in range(5):
      ...
      self.get_logger().info(f"Publish empty elevator DOWN tigger ({i + 1} / 5)")
      time.sleep(0.1)
  ...
  def loading_shelf_behavior(self):
    ...
    # Set static TF
    self.publish_cart_frame_static_once(mx_odom, my_odom, "robot_odom", "cart_frame", approch_yaw)
  ...
  def creep_backward(self, distance_m=0.75, speed_mps=0.2):
    ...
    duration_sec = (distance_m / speed_mps) + 4.0
